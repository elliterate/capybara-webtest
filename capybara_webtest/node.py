...
class Node(Base):
    ...
    def unnormalized_text(self, check_ancestor_visibility=True):
        if not self._string_node._visible(check_ancestor_visibility):
            ...
            parts = ([self.native.text] +
                     list(chain(*([Node(self.driver, c).unnormalized_text(check_ancestor_visibility=False), c.tail]
    ...
    def click(self):
        ...
        elif (self.tag_name == "input" and self["type"] in ["submit", "image"]) or (self.tag_name == "button"):
            associated_form = self._form
            if associated_form is not None:
                from capybara_webtest.form import Form
